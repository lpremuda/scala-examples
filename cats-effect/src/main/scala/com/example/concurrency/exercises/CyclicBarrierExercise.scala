...
object CyclicBarrierExercise extends IOApp.Simple {
  ...
  */
  ...
  object CBarrier {
    ...
    def createNewSignal(): IO[Deferred[IO, Unit]] = Deferred.apply[IO, Unit]
      ...
      for {
        ...
        new CBarrier {
          override def await: IO[Unit] = {
            createNewSignal().flatMap { newSignal =>
              state.modify {
                case State(1, waiting) => State(count, newSignal) -> waiting.complete(()).void
                case State(n, waiting) => State(n - 1, waiting) -> waiting.get
              }.flatten
            }
          }
        }
